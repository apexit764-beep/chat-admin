...
export default function Campaigns(): JSX.Element {
  ...
  const columns: Column<Campaign>[] = [
    ...
    { key: 'target', header: 'المستهدفون', accessor: (r) => r.targetCount, cell: (r) => r.targetCount.toLocaleString('ar-OM') },
    { key: 'sent', header: 'تم الإرسال', accessor: (r) => r.sentCount, hideOn: 'lg', cell: (r) => r.sentCount.toLocaleString('ar-OM') },
    {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5 page-fade">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="رسائل مُرسلة" value={stats.sent.toLocaleString('ar-OM')} icon={<Send className="h-5 w-5" />} iconBg="bg-success/15" iconColor="text-success" />
      ...
      <Modal
        ...
        footer={
          <>
            <button onClick={() => setPreview(form.message || 'الرسالة فارغة')} className="h-10 px-4 rounded-full border border-border-light dark:border-border-dark text-small font-medium hover:bg-bg-light dark:hover:bg-bg-dark flex items-center gap-2">
              <Eye className="h-4 w-4" /> معاينة
            </button>
            <button onClick={() => setModalOpen(false)} className="h-10 px-4 rounded-full border border-border-light dark:border-border-dark text-small font-medium hover:bg-bg-light dark:hover:bg-bg-dark">إلغاء</button>
      ...
      >
        <div className="space-y-4">
          <div>
            <Input label="اسم الحملة" value={form.name} onChange={(e) => { setForm({ ...form, name: e.target.value }); setErrors({ ...errors, name: undefined }); }} error={errors.name ?? undefined} placeholder="مثال: عروض شهر رمضان" />
          </div>
          <div>
            <Textarea label="نص الرسالة" value={form.message} onChange={(e) => { setForm({ ...form, message: e.target.value }); setErrors({ ...errors, message: undefined }); }} error={errors.message ?? undefined} placeholder="اكتب رسالتك هنا... يمكنك استخدام {{اسم_العميل}}" rows={5} />
            <p className="text-[10px] text-muted-light dark:text-muted-dark mt-1 text-end">{form.message.length} / 1024 حرف</p>
          </div>
          ...
          {form.schedule === 'later' && (
            <Input type="datetime-local" label="تاريخ ووقت الإرسال" value={form.scheduledAt} onChange={(e) => setForm({ ...form, scheduledAt: e.target.value })} icon={<Calendar className="h-4 w-4" />} />
          )}
          <div className="bg-primary/10 border border-primary/20 rounded-card p-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-h2 font-bold text-primary">{targetCount().toLocaleString('ar-OM')}</p>
            </div>
          </div>
        </div>
      </Modal>
