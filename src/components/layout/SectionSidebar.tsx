...
function SectionHeader({ children }: { children: ReactNode }): JSX.Element {
  return (
    <div className="px-4 py-3 flex items-center justify-between border-b border-border-light dark:border-border-dark">
...
function InboxSectionSidebar(): JSX.Element {
  ...
  return (
    <aside className="w-[240px] flex-shrink-0 h-screen sticky top-0 bg-white dark:bg-surface-dark border-l border-border-light dark:border-border-dark flex flex-col z-20">
      <SectionHeader>صندوق الوارد</SectionHeader>
...
function SettingsSectionSidebar(): JSX.Element {
  ...
  const items: { key: string; label: string; icon: ReactNode }[] = [
    ...
    { key: 'whatsapp', label: 'واتساب API', icon: <Smartphone className="h-4 w-4" /> },
    ...
    { key: 'billing', label: 'الفوترة', icon: <CreditCard className="h-4 w-4" /> },
