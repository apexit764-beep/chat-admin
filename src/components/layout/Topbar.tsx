    ...
    <>
      <header className="h-[56px] bg-white dark:bg-surface-dark border-b border-border-light dark:border-border-dark sticky top-0 z-10 flex items-center px-4 lg:px-6 gap-3">
        <div className="flex items-center gap-3 flex-shrink-0 min-w-0">
          ...
          {client && (
            <span className="hidden lg:inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-whatsapp/10 text-whatsapp text-[10px] font-bold uppercase tracking-wider">
              <Sparkles className="h-3 w-3" />
              {client.companyName}
              {plan && <span className="text-muted-light dark:text-muted-dark"> · {plan.nameAr}</span>}
            </span>
          )}
        </div>
